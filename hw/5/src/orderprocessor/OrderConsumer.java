...
public class OrderConsumer implements MessageListener {
    ...
    public void onMessage(Message msg) {
        try {
            ...
            System.out.println("[order-processor] Order received: " + msgText);
            System.out.println("[order-processor] >> processing order...");
            
            if (msgText.startsWith("[booking]")){
            	System.out.println("[order-processor] >> sending booking order "+msgText);
            	new MessageSender().send(Constants.NAME_QUEUE_BOOKINGS, msgText);
            	System.out.println("[order-processor] >> booking order sent.");
            } else if (msgText.startsWith("[new-trip]")){
            	System.out.println("[order-processor] >> sending new trip order "+msgText);
            	new MessageSender().send(Constants.NAME_QUEUE_NEWTRIPS, msgText);
            	System.out.println("[order-processor] >> new trip order sent.");
            } else {
            	System.out.println("Unknown order type.");
            }
            
        } catch (JMSException jmse) {
            System.err.println("[order-processor] An exception occurred: " + jmse.getMessage());
        } catch (Exception e) {
    ...
    public void receive(String queueName) throws Exception {
        ...
        System.out.println("[order-processor] Connected to " + queue.toString() + ", receiving messages...");
        try {
            synchronized (this) {
                while (true) {
                    this.wait();
