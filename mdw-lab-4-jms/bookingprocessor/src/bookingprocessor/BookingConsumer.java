...
public class BookingConsumer implements MessageListener {
    ...
    public void onMessage(Message msg) {
        try {
            ...
            System.out.println("[processor] Booking Received: " + msgText);
            System.out.println("[processor] >> processing booking request...");
            
            String confirmationText = "[Confirmation]{"+msgText+"}";
            System.out.println("[processor] Sending confirmation: " + confirmationText);
            sendConfirmation(confirmationText);
            
        } catch (JMSException jmse) {
            System.err.println("[processor] An exception occurred: " + jmse.getMessage());
        } catch (Exception e) {
    ...
    public void receive(String queueName) throws Exception {
        ...
        System.out.println("[processor] Connected to " + queue.toString() + ", receiving messages...");
        try {
            synchronized (this) {
                while (true) {
                    this.wait();
    ...
    protected void sendConfirmation(String message) throws Exception{
    	ConfirmationSender confirmationSender = new ConfirmationSender();
    	confirmationSender.send("jms/mdw-queue-confirmation", message);
    }
}
